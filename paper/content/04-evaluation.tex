...
The seed comments used to start the synthetic conversations were split into two political subjects; racism on the basis of racial identity, and on the basis of sexual orientation. Table \ref{tab:user-sdb-behavior} presents the expected and actual behavior of synthetic users from different \acp{SDB} across the two polarized topics. On the one hand, it's clear that \acp{SDB} significantly alter the behavior of our synthetic users. On the other hand, our observations reveal notable deviations from expected behaviors.
